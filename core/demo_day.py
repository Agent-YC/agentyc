...
def simulate_demo_day(
    spec: "AgentSpec", eval_result: "EvalResult", ollama: "OllamaClient"
) -> DemoDayResult:
    ...
    investors_system = _load_prompt(
        INVESTORS_PROMPT_FILE, "You are YC investors. Evaluate this pitch."
    )
    investors_prompt = f"Agent Spec:\n```yaml\n{spec_text}\n```\n\nEval: {eval_scores}\n\nFounder's Pitch:\n---\n{pitch}\n---\nGive me your feedback (JSON):"

    investors_raw = ollama.generate(
        investors_prompt, system=investors_system, temperature=0.4, format="json"
    )
